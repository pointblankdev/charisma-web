...
const EqualizeDialog: React.FC<EqualizeDialogProps> = ({ pool, onClose }) => {
  ...
  const handleEqualize = useCallback(() => {
    ...
    doContractCall({
      ...
      functionArgs: [
        uintCV(sellAmount),
        contractPrincipalCV(
          sellToken.contractAddress.split('.')[0],
          sellToken.contractAddress.split('.')[1]
        ),
        contractPrincipalCV(
          buyToken.contractAddress.split('.')[0],
  ...
  return (
    <DialogContent className="sm:max-w-[425px]">
      ...
      <div className="py-4">
        <p>
          Current Ratio: {numeral(pool.reserves.token0 / pool.reserves.token1).format('0,0.0000')}
        </p>
        <p>
          Required Action: Buy {numeral(trade.buyAmount).format('0,0.0000')} {trade.buyToken}
        </p>
        <p className="text-sm text-gray-500">
          (This will sell approximately {numeral(trade.sellAmount).format('0,0.0000')}{' '}
          {trade.sellToken})
        </p>
      </div>
      <DialogFooter>
