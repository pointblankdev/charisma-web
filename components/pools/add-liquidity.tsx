...
const LiquidityDialog = ({
  ...
  const getTokenBalance = useCallback(
    (token: TokenInfo) => {
      if (token.symbol === 'STX') {
        ...
        return (
          getBalanceByKey(getKeyByContractAddress(token.contractAddress)) / 10 ** token.decimals
        );
      }
    },
  ...
  const handleRemoveLiquidity = useCallback(() => {
    ...
    const minAmount0 = BigInt(Math.floor(parseFloat(amount0) * 0.8 * 10 ** pool.token0.decimals)); // 20% slippage
    const minAmount1 = BigInt(Math.floor(parseFloat(amount1) * 0.8 * 10 ** pool.token1.decimals)); // 20% slippage
      ...
      <div className="grid gap-4 py-4">
        ...
        <div className="grid items-center grid-cols-4 gap-4">
          ...
          <div className="col-span-3">
            <div className="flex justify-between">
              <span>
                {parseFloat(amount0).toFixed(pool.token0.decimals)} {pool.token0.symbol}
              </span>
        ...
        <div className="grid items-center grid-cols-4 gap-4">
          ...
          <div className="col-span-3">
            <div className="flex justify-between">
              <span>
                {parseFloat(amount1).toFixed(pool.token1.decimals)} {pool.token1.symbol}
              </span>
              <span className="text-sm text-gray-500">
