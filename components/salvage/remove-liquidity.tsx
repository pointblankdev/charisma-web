...
const RemoveLiquidityFromIndex = ({
  amount,
  address,
  metadata,
  indexWeight
  ...
  function salvage() {
    const postConditions = [
      Pc.principal(sender)
        .willSendEq(Number(tokens) * Number(indexWeight))
        .ft(address, metadata.ft),
      ...metadata.contains.map((item: any) =>
        Pc.principal(address)
          .willSendEq(Number(tokens) * Number(item.weight))
          .ft(item.address, item.ft)
      )
    ];
  ...
  return (
    <Button
      disabled={tokens <= 0}
      variant="ghost"
      className="text-primary hover:bg-white hover:text-primary z-30"
      onClick={salvage}
    >
      Remove Liquidity
    </Button>
