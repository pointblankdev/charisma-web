...
export async function getCollectionSize(contract: string) {
...
export async function getNftOwner(contract: string, tokenId = 99) {
  // Create a unique cache key
...
export async function getTokenMetadata(contract: string) {
  try {
    const [address, name] = contract.split('.');
    ...
    const url = cvToJSON(hexToCV(response.data.result))?.value?.value?.value;
    console.log(url);
    ...
    return {
      ...
      symbol: 'ERR',
      decimals: 6,
      image: '/sip9/pills/red-pill-spin.gif',
      description: 'Error loading metadata'
    };
  }
}

export async function getTokenImage(contract: string) {
  const [address, name] = contract.split('.');
...
export async function getIdentifier(contract: string) {
...
export async function getTokenName(contract: string) {
