...
export type SwapContextType = {
  tokenList: any;
  setTokenList: Dispatch<SetStateAction<any>>;
...
const stakingConfigs = [
  {
    ...
    to: 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.liquid-staked-welsh-v2',
    rate: 1 / 1.0005
  },
  {
    from: 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token',
    to: 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.wrapped-charisma',
    rate: 1
  },
  {
    from: 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token',
    to: 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.liquid-staked-charisma',
    rate: 1
  }
];
...
function findPool(tickers: Ticker[], fromToken: string, toToken: string): Ticker | undefined {
  return tickers.find(
    ticker =>
      ticker.ticker_id === `${fromToken}_${toToken}` ||
      ticker.ticker_id === `${toToken}_${fromToken}`
  );
}

function calculateSwapAmount(
  pool: Ticker | undefined,
  fromAmount: number,
  fromToken: string,
  toToken: string
): number {
...
function getStakingPair(toToken: string): StakingConfig {
  ...
  if (!config) {
    new Error(`No staking configuration found for: ${toToken}`);
  }
...
export function reverseSwapConfig(swapConfig: SwapConfig) {
  const reversedSteps: StepConfig[] = swapConfig.steps
    .map(step => ({
      action:
        step.action === 'STAKE' ? 'UNSTAKE' : step.action === 'UNSTAKE' ? 'STAKE' : step.action,
