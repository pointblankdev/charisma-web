...
export default function Swap({ data }: Props) {
  ...
  useEffect(() => { setTokenList(data.tokens) }, [setTokenList, data.tokens]);
...
const ExecuteStrategy = () => {
  ...
  const amountIn = Number(swapConfig.steps[0].fromAmount * 1000000)

  const firstAction = swapConfig.steps[0].action
