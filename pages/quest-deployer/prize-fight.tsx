...
const generateTemplate = ({ questUri }: any) => {
    return `;; Prize Fight Contract
;; This contract manages an auction system where players bid for NFTs with energy.
...
(define-data-var specialized-land-id uint u0)
(define-data-var land-bonus uint u1)
...
(define-public (tap (land-id uint) (nft-contract <nft-trait>) (cdk-contract <cdk-trait>))
    (begin
        ...
        (let
            (
                ...
                (AMOUNT (if (is-eq land-id (get-specialized-land-id)) (* bid-amount (get-land-bonus)) bid-amount))
...
(define-read-only (get-specialized-land-id)
    (var-get specialized-land-id)
)

(define-read-only (get-land-bonus)
    (var-get land-bonus)
)
...
(define-public (set-specialized-land-id (new-specialized-land-id uint))
    (begin
        (try! (is-authorized))
        (ok (var-set specialized-land-id new-specialized-land-id))
    )
)

(define-public (set-land-bonus (new-bonus uint))
    (begin
        (try! (is-authorized))
        (ok (var-set land-bonus new-bonus))
    )
)
...
export default function PrizeFightTemplate({ onFormChange }: any) {
    const defaultValues: Partial<ProposalFormValues> = {
        questUri: "https://charisma.rocks/api/quests/",
    }
