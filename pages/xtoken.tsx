...
async function getPoolReserves(
  ...
  try {
    const result: any = await fetchCallReadOnlyFunction({
      ...
      functionArgs: [principalCV(token0Address), principalCV(token1Address)],
        ...
        <div className="m-2 rounded-full sm:container sm:mx-auto sm:py-10 md:max-w-5xl">
          ...
          {hasEnoughExperience && (
            <div title={`This feature is unlocked for users with over 100 experience points.`}>
              <div className="w-full pt-8 text-3xl font-bold text-center uppercase">
                The Mempool
              </div>
              <div className="w-full pb-8 text-center text-md text-muted/90">
                Recent transactions in the mempool.
                <p className="mt-2 text-sm text-yellow-500">✨ Experience Bonus Feature</p>
              </div>
              <Card className="p-0 overflow-hidden bg-black text-primary-foreground border-accent-foreground rounded-xl">
                <div className="m-2 space-y-2">
                  {_.uniqBy(recentTransactions, tx => tx.sender_address).map((tx: Transaction) => {
                    return (
                      <div
                        key={tx.tx_id}
                        className="p-4 bg-[var(--sidebar)] border border-[var(--accents-7)] rounded-lg"
                      >
                        ...
                        <div className="mt-2">
                          Receipt Time: {formatRelativeTime(tx.receipt_time_iso)}
                        </div>
                      </div>
                    );
                  })}
  ...
  return (
    <div className="flex flex-col items-center overflow-hidden">
      <Image
        ...
        width={300}
        height={300}
        className="transition-all scale-150 translate-y-24 cursor-pointer sm:hidden"
      />
      <div className="flex w-full pt-24 pb-8 px-8 sm:p-24 sm:pb-0 my-[10vh] bg-[var(--sidebar)] border border-[var(--accents-7)] rounded-lg sm:rounded-lg mt-12">
        <div className="flex-col items-center hidden w-full space-y-4 sm:w-64 sm:flex">
          <Image
            ...
            width={300}
            height={300}
            className="transition-all -translate-x-12 -translate-y-20 cursor-pointer scale-[2]"
          />
        ...
        <div className="flex flex-col items-center justify-center w-full px-4 text-lg text-center -translate-y-16 sm:text-md sm:text-start sm:items-start sm:justify-start sm:px-0">
          <div className="flex items-baseline justify-center w-full text-center sm:justify-start">
            <div className="py-4 text-6xl sm:py-0">Charisma</div>
            <div className="text-lg font-bold text-primary">CHA</div>
          </div>
          <div className="mt-4 text-lg grow text-secondary/80">
            Introducing the new Charisma token– stability, security, swaps.
          </div>
          <div className="mt-8 text-md text-secondary/80">
            The token contract limits how many tokens can be wrapped per transaction, and how many
            wrapping transactions can occur in a set of blocks.
          </div>
          <div className="mt-8 text-md text-secondary/80">
            This ensures fair access for all users and adds an exciting element of competition to
...
const StatsSection = ({ data }: Props) => {
  ...
  const stat2Message =
    charismaTokenStats.blocksPerTransaction === 1
      ? charismaTokenStats.transactionsAvailable === 0
        ? '100%'
        : (Number(1 / (Number(charismaTokenStats.transactionsAvailable) + 1)) * 100).toFixed(2) +
          '%'
      : charismaTokenStats.blocksPerTransaction;
  const profitMargin = (
    (data.chaPrice * charismaTokenStats.tokensPerTransaction) / 10 ** 6 -
    (data.tokenPrices['STX'] * highestBid) / 10 ** 6
  ).toFixed(2);
  ...
  return (
    <div>
      ...
      <div className="w-full pb-8 text-center text-md text-muted/90">
        Bid in the mempool to wrap your tokens.
      </div>
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div
          className={`flex flex-col items-center justify-center ${
            highestBid === 0 ? 'col-span-2' : 'col-span-1'
          } p-4 space-y-2 rounded-lg text-md bg-[var(--sidebar)] border border-[var(--accents-7)]`}
        >
          <div className="flex items-center space-x-0">
            <div className="-mr-0 text-4xl font-semibold">
              {charismaTokenStats.tokensPerTransaction / Math.pow(10, 6)}
            </div>
            <Image
              src={charismaSquare}
              alt="DMG Logo"
              width={64}
              height={64}
              className="inline w-7 h-7 rounded-full translate-x-1.5 translate-y-0.5"
            />
          </div>
        ...
        {highestBid > 0 && (
          <div className="flex flex-col items-center justify-center p-4 space-y-2 rounded-lg text-md bg-[var(--sidebar)] border border-[var(--accents-7)]">
            <div className="flex items-center space-x-0">
              <div className="-mr-0 text-4xl font-semibold">
                {(Math.ceil((highestBid / 10 ** 6) * 100) / 100).toFixed(2)}
              </div>
              <Image
                src={stxLogo}
                alt="DMG Logo"
        ...
        {isUnlocked ? (
          <div
            className={`flex flex-col items-center justify-center p-4 space-y-2 rounded-lg text-md bg-[var(--sidebar)] border border-[var(--accents-7)]`}
          >
...
const WrappingSection = ({ data }: Props) => {
      ...
      doContractCall({
        ...
        postConditions: [
          Pc.principal(stxAddress)
            .willSendEq(amountIn)
            .ft('SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token', 'charisma')
        ] as any[]
      });
    }
  };
  ...
  return (
    <div className="mt-20 mb-12">
      ...
      <div className="w-full pb-8 text-center text-md text-muted/90">
        Convert your governance tokens into Charisma tokens.
      </div>
      <div className="container flex h-48 p-6 space-x-2 rounded-2xl max-w-prose bg-[var(--sidebar)] border-t-2 border-[var(--accents-7)] leading-0">
        <div className="w-full">
          <div className="font-bold text-center text-muted-foreground">Available</div>
          <div className="flex items-center justify-center mt-2 space-x-1 font-bold text-center">
            ...
            <div className="pb-1">
              <Image
                src={wallet.charisma.balance > 0 ? dmgLogoPulse : dmgLogo}
                alt="DMG Logo"
                width={20}
                height={20}
                className="inline"
        ...
        <div className="w-full">
          ...
          <div className="mt-2 font-bold text-center">{numeral(data.chaPrice).format('$0.00')}</div>
        </div>
        <div className="w-full">
          <div className="font-bold text-center text-muted-foreground whitespace-nowrap">
            Total Value
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p className="max-w-xs text-sm">
                    This value represents your DMG token balances at their post-wrap valuation using
                    current CHA prices.
                  </p>
                </TooltipContent>
              </Tooltip>
            ...
            {wallet.redPilled ? (
              ...
              <div className="flex items-center mt-2 space-x-2 font-bold">
                ...
                {!charismaClaims.hasClaimed && (
                  <Button
                    onClick={handleMintRedPill}
                    className="h-6 px-2 font-normal rounded-full cursor-pointer bg-primary"
                  >
                    Mint
                  </Button>
                )}
          ...
          {wallet.redPilled ? (
            <div className="">
              <div className="flex justify-between py-2">
                ...
                <div
                  ...
                  className="px-2 rounded-full cursor-pointer bg-primary"
                >
                  Max
                </div>
              </div>
            ...
            <div className="text-center">
              ...
              {!charismaClaims.hasClaimed ? (
                <Button
                  onClick={handleMintRedPill}
                  className="px-4 py-2 font-light rounded-full cursor-pointer bg-primary"
                >
                  Mint Red Pill NFT
                </Button>
              ) : (
                <div>
                  <p className="my-12 text-lg font-semibold">
                    You have already made a NFT claim for this wallet address.
                  </p>
                </div>
              )}
            </div>
