  ...
  try {
    ...
    const poolsWithTokens = await Promise.all(
      pools.map(async pool => {
        ...
        if (req.query.ai) {
          return {
            ...
            token0: {
              ...
              price: tokenPrices[token0.symbol] / 10 ** token0.decimals || 0,
              tvl: (reserves.token0 / 10 ** token0.decimals) * tokenPrices[token0.symbol],
              isLpToken: token0.isLpToken || false
            },
            token1: {
              contractAddress: token1.contractAddress,
              price: tokenPrices[token1.symbol] / 10 ** token1.decimals || 0,
              tvl: (reserves.token1 / 10 ** token1.decimals) * tokenPrices[token1.symbol],
              isLpToken: token0.isLpToken || false
            },
        ...
        const tvl =
          (reserves.token0 / 10 ** token0.decimals) * token0Price +
