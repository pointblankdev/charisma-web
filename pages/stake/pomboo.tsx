...
import StakePombooButton from '@components/stake/stake-pomboo';
import UnstakePombooButton from '@components/stake/unstake-pomboo';
import liquidPomboo from '@public/liquid-pomboo-21.png'
...
export default function Stake({ data }: Props) {
  ...
  const meta = {
    title: 'Charisma | Stake Pomboo Tokens',
    description: META_DESCRIPTION,
    image: '/liquid-pomboo-21.png'
  };

  const handleTokenAmountChange = (event: any) => {
    const { value } = event.target;
    // Limit input to only allow numbers and to 8 decimal places
      ...
      <Layout>
        <div className="m-2 sm:container sm:mx-auto sm:py-10 md:max-w-2xl">
          ...
          <Card className='p-0 overflow-hidden bg-black text-primary-foreground border-accent-foreground rounded-xl'>
            <Image alt='Liquid Pomboo' src={liquidPomboo} width="1080" height="605" className='border-b border-accent-foreground' />
            <div className='m-2'>
              ...
              <div className='flex justify-between'>
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent className='max-w-2xl leading-tight text-white bg-black border-primary'>
                      ...
                      <p className="mb-4">
                        Staking your Pomboo tokens allows you to participate in governance and earn staking rewards generated from network activities. Your staked tokens help secure the network and in return, you earn more tokens over time.
                      </p>
                      <p className="mb-4">
                ...
                <div className='px-2 sm:p-0 sm:px-4'>
                  {millify(tokensInPool / 100000000)} POMBOO Staked
                </div>
              </div>
              <p className="mb-8 text-xs font-thin leading-tight sm:text-sm">
                Pomboo Staking is a crucial part of the network's financial ecosystem, providing a way for token holders to earn passive income while contributing to the token's number-go-up mechanism.
              </p>
