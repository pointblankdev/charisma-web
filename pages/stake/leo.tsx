...
import StakeLeoButton from '@components/stake/stake-leo';
import UnstakeLeoButton from '@components/stake/unstake-leo';
...
export default function StakeLeo({ data }: Props) {
  const [tokenAmount, setTokenAmount] = useState('');

  const meta = {
    title: 'Charisma | Stake Leo Tokens',
    description: META_DESCRIPTION,
    image: '/liquid-leo.png'
  };
      ...
      <Layout>
        <div className="m-2 sm:container sm:mx-auto sm:py-10 md:max-w-2xl">
          ...
          <Card className='p-0 overflow-hidden bg-black text-primary-foreground border-accent-foreground rounded-xl'>
            <Image alt='Liquid Leo' src={liquidLeo} width="1080" height="605" className='border-b border-accent-foreground' />
            <div className='m-2'>
              <div className='flex justify-between mb-2'>
                <h1 className="self-center font-bold text-md sm:text-2xl">Liquid Staked Leo</h1>
                {data.exchangeRate && <div className="self-center px-2 my-1 text-xs font-light text-center rounded-full sm:text-lg sm:p-0 sm:px-4">
                  <div className="self-center px-2 my-1 text-xs font-light text-center rounded-full sm:text-lg bg-primary sm:p-0 sm:px-4">1 sLEO = {Number(data.exchangeRate) / 1000000} LEO</div>
                </div>}
              </div>
              ...
              <div className='flex justify-between'>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className='max-w-2xl leading-tight text-white bg-black border-primary'>
                      ...
                      <ul className="pl-5 mb-4 space-y-2 list-disc text-md">
                        <li>
                          <b>Stake Tokens</b>: Stake your Leo tokens to receive Liquid Staked Leo (sLEO). The amount of sLEO you receive is calculated based on the current inverse exchange rate.
                        </li>
                        <li>
                          <b>Unstake Tokens</b>: Redeem your sLEO for Leo tokens based on the current exchange rate.
                        </li>
                      </ul>
                      <p className="mb-4">
                        Staking your Leo tokens allows you to participate in governance and earn staking rewards generated from network activities. Your staked tokens help secure the network and in return, you earn more tokens over time.
                      </p>
                      <p className="mb-4">
                ...
                <div className='px-2 sm:p-0 sm:px-4'>
                  {millify(tokensInPool / 1000000)} LEO Staked
                </div>
              </div>
              <p className="mb-8 text-xs font-thin leading-tight sm:text-sm">
                Leo Staking is a crucial part of the network's financial ecosystem, providing a way for token holders to earn passive income while contributing to the token's number-go-up mechanism.
              </p>
              <div className='space-y-2'>
                ...
                <div className='flex space-x-1'>
                  <StakeLeoButton tokens={tokenAmount} />
                  <UnstakeLeoButton tokens={tokenAmount} />
                </div>
              </div>
            </div>
          </Card>
