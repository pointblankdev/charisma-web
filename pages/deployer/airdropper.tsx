...
const generateTemplate = (parsedAddresses: any[]) => {
    ...
    const wrappedChunks = chunkedAddresses.map((chunk) => {
        const addressList = chunk.map(item =>
            `   {to: '${item.address}, amount: u${item.amount}, memo: none}`
        ).join('\n');
...
export default function AirdropTemplate({ onFormChange }: { onFormChange: (template: string) => void }) {
