...
const DEFAULT_TOKEN_AMOUNT = 1800000;
  ...
  const wrappedChunks = chunkedAddresses
    .map(chunk => {
      const addressList = chunk
        .map(item => `   (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.energy-storage store-energy '${item.address} u${item.amount}))`)
        .join('\n');
...
export default function AirdropEnergyTemplate({
  onFormChange
}: {
