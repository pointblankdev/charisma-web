...
export const getStaticProps: GetStaticProps<any> = async ({ params }) => {
  ...
  const data: DataPoint[] = Object.keys(groupedSwaps).map(blockHeight => {
    ...
    const prices = swaps.map((swap: any) =>
      poolData.token1 === swap['token-out']
        ? swap['amt-in-adjusted'] / swap['amt-out']
        : swap['amt-out'] / swap['amt-in-adjusted']
    );
    const volume = swaps.reduce(
      (acc: number, swap: any) =>
        Number(acc) +
        ((Number(swap['amt-in']) / 10 ** getDecimalsByContract(swap['token-in'])) *
          getPriceByContract(swap['token-in']) +
          (Number(swap['amt-out']) / 10 ** getDecimalsByContract(swap['token-out'])) *
            getPriceByContract(swap['token-out'])),
      0
    );
    const dataPoint = {
      ...
      <Layout>
        ...
        >
          ...
          >
            <div>Pool: {symbol}</div>
            <div>
              Current Price: {currentPrice.toFixed(2)} {symbol.split('-')[0]}
            </div>
            <div>
