...
(define-public (swap (rulebook <rulebook-trait>) (forward bool) (amt-in uint))
  (let (
    (reserve-in (unwrap-panic (if forward (contract-call? .dme000-governance-token get-balance CONTRACT) 
    ...
    (reserve-out (unwrap-panic (if forward (contract-call? .hooter-the-owl get-balance CONTRACT) 
      (contract-call? .dme000-governance-token get-balance CONTRACT))))
    ...
    (try! (if forward (contract-call? .dme000-governance-token transfer amt-in tx-sender CONTRACT none) 
    ...
    (try! (as-contract (if forward (contract-call? .hooter-the-owl transfer amt-out CONTRACT tx-sender none)
      (contract-call? .dme000-governance-token transfer amt-out CONTRACT tx-sender none))))
    
    (ok {amt-in: amt-in, amt-out: amt-out})))

(define-public (mint (who principal) (amount uint))
  (let (
    (reserve0 (unwrap-panic (contract-call? .dme000-governance-token get-balance CONTRACT)))
    ...
    (try! (contract-call? .dme000-governance-token transfer token0-amount who CONTRACT none))
...
(define-public (burn (who principal) (amount uint))
  (let (
    ...
    (try! (as-contract (contract-call? .dme000-governance-token transfer token0-amount CONTRACT who none)))
...
(define-read-only (get-tokens)
  (ok {token0: .dme000-governance-token, token1: .hooter-the-owl}))
...
(define-read-only (get-reserves)
  (ok {
    token0: (unwrap-panic (contract-call? .dme000-governance-token get-balance CONTRACT)),
