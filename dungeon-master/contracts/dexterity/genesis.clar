...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? GENESIS amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance GENESIS who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply GENESIS)))
...
(define-public (swap (amount uint) (opcode (optional (buff 16))))
    (let (
        ...
        (try! (if is-a-in
            (contract-call? .token-a transfer amount sender CONTRACT opcode)
            (contract-call? .token-b transfer amount sender CONTRACT opcode)))
        ;; Transfer output token to sender - keeping contract traits inline
        (try! (as-contract (if is-a-in
            (contract-call? .token-b transfer (get dy delta) CONTRACT sender opcode)
            (contract-call? .token-a transfer (get dy delta) CONTRACT sender opcode))))
        (ok delta)))
...
(define-public (add-liquidity (amount uint) (opcode (optional (buff 16))))
    (let (
      ...
      (delta (calculate-liquidity-delta amount)))
        ...
        (try! (contract-call? .token-a transfer (get dx delta) sender CONTRACT opcode))
        (try! (contract-call? .token-b transfer (get dy delta) sender CONTRACT opcode))
        ;; Mint LP tokens to sender
        (try! (ft-mint? GENESIS (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint) (opcode (optional (buff 16))))
    (let (
      ...
      (delta (calculate-liquidity-delta amount)))
        ...
        ;; (try! (ft-burn? GENESIS (get dk delta) tx-sender))
        ;; Transfer tokens back to sender
        (try! (as-contract (contract-call? .token-a transfer (get dx delta) CONTRACT sender opcode)))
        (try! (as-contract (contract-call? .token-b transfer (get dy delta) CONTRACT sender opcode)))
        (ok delta)))
...
(define-private (get-reserves)
    { 
      a: (unwrap-panic (contract-call? .token-a get-balance CONTRACT)), 
      b: (unwrap-panic (contract-call? .token-b get-balance CONTRACT)) 
    })
