...
(define-read-only (apply (base-amount uint) (target principal) (caller principal))
   (let (
       ;; Calculate bonuses
       (happy-bonus (if (> (contract-call? .happy-welsh get-balance target) u0)
                       (var-get happy-welsh-bonus)
                       u0))
       (weird-bonus (if (> (contract-call? .weird-welsh get-balance target) u0)
                       (var-get weird-welsh-bonus)
                       u0))
       (punk-bonus (if (> (contract-call? .welsh-punk get-balance target) u0)
