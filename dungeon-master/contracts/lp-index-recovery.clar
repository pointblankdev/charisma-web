...
(define-public (get-claim-amount-lp (user principal))
  (let
  (
    (lp-amounts (unwrap-panic (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.back-to-the-charisma check-lp user)))
...
(define-public (mint-a)
  (let (
    ...
    (if (and (> amount u0) (check-and-set-claimed-a user))
      (contract-call? .dme000-governance-token dmg-mint amount user)
