;; Charisma Token Redemptions: WELSH
...
(define-constant max-claim-amount u1000)
...
(define-read-only (is-blue-pilled)
  (ok (asserts! (contract-call? .the-blue-pill has-balance tx-sender) err-not-blue-pilled))
)

(define-read-only (get-available-tokens)
...
(define-public (redeem-tokens (amount uint))
  (let (
    (available-tokens (unwrap-panic (get-available-tokens)))
    (claim-amount (if (> amount max-claim-amount) max-claim-amount amount))
